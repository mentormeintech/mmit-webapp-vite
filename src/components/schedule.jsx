...
function MentorSchedule(props) {
    ...
    const handleAddEvent = async (formData) => {
        try {
            ...
            const mentor_provider_token = "ya29.a0ARW5m74MZPHl8sZsbR4qriZMZ8C7_gKDDC9r6Rm1CpsPoBzcwk6VmtD5YPaKtWU2iOzYYWJxwem5wovFDX12weS-5TRZ34reAnVzOjQ5YfpU0exFE5Aoqofyu4gyThM_RGJxvgUhbVn2SCu4KTWNzmw_VexbYm4pkwaCgYKAY4SARASFQHGX2MiLfnG2XM4zEnQn9vNvZt4Qg0169";
            if (description) {
                setloading(true)
                const newEvent = {
                    title: title || 'available',
                    // mentor_google_id: session.user.id,
                    mentor_google_id: 'session.user.email',
    ...
    const handleLogin1 = async (event) => {
        ...
        try {
            ...
            if (error) {
                ...
                localStorage.setItem("handleLogin", JSON.stringify(data))
                getMyEvents(data);
            }
        } catch (error) {

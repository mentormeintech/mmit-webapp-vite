			...
			if (response && response?.success === true) {
				if (career_id === "all") {
					...
					const filteredMentors = mentorData.docs.filter(
						(mentor) =>
							mentor?.area_of_expertise?.[0]?._id === career_id
					);
		...
		<>
			{loading ? (
				...
				<div className="justify-center overflow-hidden m-auto w-[100%]">
					...
					<div className="ml-[10px] sm:ml-[100px] mt-[100px] mb-[20px] font-bold text-[20px] sm:text-[30px]">
						MENTORS
					</div>
          <div className="ml-[10px] sm:ml-[100px]  mb-[5px] font-medium text-[20px] sm:text-[30px]">Get connected to the best mentors</div>
          <div className="ml-[10px] sm:ml-[100px]  sm:w-[50%] mb-[30px] text-[#434343] font-[500] text-[13px] sm:text-[17px]">Book and meet over 1k mentors for 1:1 mentors in our global Communtiy</div>
					<div className="flex flex-row ml-[10px] mr-[10px] sm:mr-0  sm:ml-[100px] items-center">
						<div className="relative z-10 w-[100%] mr-[10px] sm:mr-[40px] sm:w-[70%] flex items-center pl-[5px] border-b-2">
							<img
								...
								className="absolute z-40 w-[25px] sm:ml-[20px] sm:w-8"
								alt=""
							/>
							<input
								type="text"
								className="sm:h-16 h-10 w-full rounded-sm pr-[15px] pl-[30px] sm:pl-16 sm:pr-10 outline-none"
						...
						{data?.docs &&
							data?.docs?.map((mentor, index) => (
								<ProfileCard
									...
									reviews={
										mentor.ratings && mentor.ratings?.length
									}
									experience={

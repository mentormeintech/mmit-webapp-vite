...
export default function MenteeCareer() {
    ...
    async function createCareer(event) {
        try {
            ...
            if (!careerPath) {
                ...
                const response = await putRequest(`mentee/career`, formData)
                if (response && response.success === true) {
    ...
    return (
        <>
            {loading ? <Spinner /> : <>
                <div>
                    <SignupHeader />
                    <CareerPath careers={careers} setcareerPath={setcareerPath} careerPath={careerPath} createCareer={createCareer} loading={careerLoading} userType={"mentee"} />
                </div>
                <Footer />
            </>}
        </>
